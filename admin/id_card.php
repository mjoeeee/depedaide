...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../asset/css/admin_id_card.css">
...
<body>
    ...
    <div class="container">
        ...
        <a href="dashboard" >
                    ...
                    <a href="id_card"   style="font-weight: bold; color:rgb(238, 108, 21);">
                ...
                <tbody>
                    <?php foreach ($results as $row): ?>
                        ...
                        <td><?= htmlspecialchars($row['request_id']) ?></td>
                            ...
                            <td><?= htmlspecialchars($row['hrId']) ?></td>
                            <td style="
                                font-weight:bold;
                                padding: 5px 10px; 
                                border-radius: 5px; 
                            ...
                            <td>
                                    ...
                                    <button class="btn-view" onclick="redirectToCard('<?=urlencode($row['request_id'])?>')">
                                        <i class="fas fa-eye"></i>
                                    </button>


                                    <button class="btn-print_icon" data-row-id="<?= $row['request_id'] ?>">
                                        <i class="fas fa-print"></i>
                                    </button>
                                    <div class="hidden print-buttons" style="margin-top: 5px;">
    <button class="btn-print" onclick="openEODBID('<?=urlencode($row['request_id'])?>')">EODB ID</button>
    <button class="btn-print" onclick="openEODBIDBB('<?=urlencode($row['request_id'])?>')">EDODB ID BB</button>
    <button class="btn-print" onclick="openPOCKETID('<?=urlencode($row['request_id'])?>')">Pocket ID</button>
</div>
    ...
    <script>
$(document).on("click", ".btn-print_icon", function () {
    var buttons = $(this).closest("td").find(".print-buttons");
    $(".print-buttons").not(buttons).addClass("hidden"); // Hide others
    buttons.toggleClass("hidden"); // Toggle only current
});
...
$(".btn-edit").click(function () {
    ...
    <script>
        ...
        function redirectToCard(requestId) {
                window.location.href = 'view_card?request_id=' + requestId;
                }
        function openEODBID(requestId) {
                window.open('eodb_id?request_id=' + requestId, '_blank');
                }
        function openEODBIDBB(requestId) {
                window.open('eodb_idBB?request_id=' + requestId, '_blank');
                }
        function openPOCKETID(requestId) {
                window.open('pocket_id?request_id=' + requestId, '_blank');
                }
    </script>
