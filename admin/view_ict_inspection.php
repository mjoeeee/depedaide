...
include '../config.php';
include 'query/view_ict_inspection.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="header-card">
            <div class="breadcrumb">
                ...
                <a href="ict_inspection">Go back</a>
                <span class="separator">/</span>
    ...
    <div class="form-container">
        ...
        <h3 style="color: #007bff; font-weight: bold; display: flex; align-items: center;">
            ...
            <span style="flex-grow: 1; border-bottom: 2px dashed #007bff; margin-left: 10px;"></span>
        </h3>

        <div class="form-row">
            <div class="form-group">
                <label for="defects">Defects</label>
                <textarea id="defects" rows="6" name="defects" placeholder="Enter Defects"><?= htmlspecialchars($result['defects'] ? $result['defects'] : '') ?></textarea>
            </div>
            <div class="form-group">
                <label for="findings">Findings and Observations</label>
                <textarea id="findings" rows="6" name="findings" placeholder="Enter Findings and Observations"><?= htmlspecialchars($result['findings'] ? $result['findings'] : '') ?></textarea>
            </div>
        </div>

        <div class="form-group">
            <label for="parts">Parts/Components to be Repaired/Replaced</label>
            <textarea id="parts" rows="4" name="parts_repair_replace" placeholder="Enter Parts/Components to be Repaired/Replaced"><?= htmlspecialchars($result['parts_repair_replace'] ? $result['parts_repair_replace'] : '') ?></textarea>
        </div>
        <h3 style="color: #28a745; font-weight: bold; display: flex; align-items: center;">
            🛠️ Post-Repair Details
            <span style="flex-grow: 1; border-bottom: 2px dashed #28a745; margin-left: 10px;"></span>
        </h3>
        ...
        <div class="additional-fields">
            <div class="form-group">
                ...
                <input type="text" id="jobOrderNo" name="job_order_no" value="<?= htmlspecialchars($result['job_order_no'] ? $result['job_order_no'] : '') ?>" placeholder="Enter Job Order No.">
            </div>
            <div class="form-group">
                <label for="amount">Amount</label>
                <input type="text" id="amount" name="amount" value="<?= htmlspecialchars($result['amount'] ? $result['amount'] : '') ?>" placeholder="Enter Amount">
            </div>
            <div class="form-group">
                <label for="invoiceNo">Invoice No.</label>
                <input type="text" id="invoiceNo" name="invoice_no" value="<?= htmlspecialchars($result['invoice_no'] ? $result['invoice_no'] : '') ?>" placeholder="Enter Invoice No.">
            </div>
        </div>

        <!-- Comment After Repair -->
        <div class="form-group">
            <label for="commentAfterRepair">Comment After Repair</label>
            <textarea id="commentAfterRepair" name="comment_after_repair" rows="4" placeholder="Enter Comment After Repair"><?= htmlspecialchars($result['comment_after_repair'] ? $result['comment_after_repair'] : '') ?></textarea>
        </div>

        <!-- Buttons Section -->
        <div class="d-flex justify-content-end gap-2 mt-3">
            <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Update Details</button>
            <a href="ict_inspection" class="btn btn-secondary"><i class="fas fa-times"></i> Cancel</a>
    </div>
    </form>
    </div>
