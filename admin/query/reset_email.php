...
try {
    ...
    $stmt = $conn->prepare("SELECT * 
                            ...
                            WHERE r.request_type_table = 'password_reset'
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["idd"])) {
    ...
    try {
        ...
        if ($row && !empty($row['attachment'])) {
            $filePath = "../" . $row['attachment']; // Adjust path if needed
        ...
        $stmt = $conn->prepare("DELETE FROM tbl_passreset_depaide WHERE id = ?");
        $stmt->execute([$idd]);
        ...
        if ($conn->inTransaction()) { // Check if a transaction is active
            $conn->rollBack(); // Rollback only if a transaction is active
        }
        echo json_encode(["success" => false, "error" => $e->getMessage()]);
        exit;
    }
