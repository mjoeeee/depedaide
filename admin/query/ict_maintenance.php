...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    $identified_problem = $_POST['identified_problem'];
    $desc_technical_support = $_POST['desc_technical_support'];
    ...
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE tbl_ictmrf_depaide SET date_inspected = ?, IPI = ?, DTS = ?, recomend = ? WHERE id = ?");
        $stmt2->execute([$date_inspected, $identified_problem, $desc_technical_support, $recomend, $id]);
...
} else if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["idd"])) {
    $request_id = $_POST['request_id'];
    $idd = $_POST['idd'];
    ...
    try {
        ...
        $stmt = $conn->prepare("DELETE FROM tbl_ictmrf_depaide WHERE id = ?");
        $stmt->execute([$idd]);
