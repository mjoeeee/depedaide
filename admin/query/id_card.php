...
try {
    $stmt = $conn->prepare("SELECT r.*, d.*, u.email, u.userId, u.role, hrId
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    try {
        ...
        // Retrieve image and sign file paths
        $stmt = $conn->prepare("SELECT image, sign FROM tbl_printingid_depaide WHERE id = ?");
        $stmt->execute([$idd]);
        $fileData = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($fileData) {
            $imagePath = "../" . $fileData['image'];
            $signPath = "../" . $fileData['sign'];
        ...
        $stmt = $conn->prepare("DELETE FROM tbl_printingid_depaide WHERE id = ?");
        $stmt->execute([$idd]);
