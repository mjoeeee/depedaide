...
$stmt = $conn->prepare("SELECT * 
                        ...
                        WHERE r.request_type_table = 'ict_equipment_inspection'
...
$parts_repair_replace = $_POST['parts_repair_replace'];
$job_order_no = $_POST['job_order_no'];
$amount = $_POST['amount'];
$invoice_no = $_POST['invoice_no'];
$comment_after_repair = $_POST['comment_after_repair'];
...
try {
    ...
    $stmt2 = $conn->prepare("UPDATE tbl_inspection_depaide SET defects=?, findings = ?, parts_repair_replace = ?, job_order_no = ?, amount = ?, invoice_no = ?, comment_after_repair = ? 
                             WHERE id = ?");
    $stmt2->execute([$defects, $findings, $parts_repair_replace, $job_order_no, $amount, $invoice_no, $comment_after_repair, $id]);
...
try {
    ...
    $stmt->execute([$request_id]);
    $stmt = $conn->prepare("DELETE FROM tbl_inspection_depaide WHERE id = ?");
    $stmt->execute([$id]);
    $conn->commit();
    echo json_encode(["success" => true]);
    exit; 
