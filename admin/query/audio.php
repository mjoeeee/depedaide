...
try {
    $stmt = $conn->prepare("SELECT * 
                            ...
                            WHERE r.request_type_table = 'audio_visual_editing'
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    try {
        ...
        if ($user) {
            ...
            try {
                ...
                $mail->Body = "
                <p>Dear $lastname,</p>
                <p>Your request (ID: <strong>$request_id</strong>) has been updated.</p>
    ...
    try {
        ...
        $stmt = $conn->prepare("DELETE FROM tbl_audiovisual_depaide WHERE id = ?");
        $stmt->execute([$idd]);
